...
module.exports.charge = async (request) => {
  ...
  const supportedCards = await OpenFeature.getClient().getObjectValue(
    'paymentSupportedCardsProblem',
    SUPPORTED_CARDS_DEFAULT
  )

  if (!supportedCards[cardType]) {
    const supportedCardNames = Object.values(supportedCards).join(' or ')

    throw new Error(`Sorry, we cannot process ${cardNiceType} credit cards. Only ${supportedCardNames} are accepted.`)
  }
  ...
  logger.info(
    { transactionId, cardType, lastFourDigits, amount: { units, nanos, currencyCode }, loyalty_level },
